...
def resolve_productsSearch(_obj, _info, query = None):
  ...
  # # :string
  location_    = query.get('location', None)
  location     = location_.strip() if location_ else None
  ...
  # filter on user:profle:location
  if location:
    pls = [p for p in pls if p.is_from_location(location)]
